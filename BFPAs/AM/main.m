...
%% Prameter  of the local or the global pollination  ----------------------
     ...
     global p
            p_values = [0.3,0.4,0.5,0.5,0.5,0.5,0.1,0.5,0.5,0.1,0.1,0.2,0.2];
...
for  w=1:indiv% ------------ loop of flower processing  global or local pollination ------
         ...
         if (rand < p)
             %% Global pollination -------------------------------------------------
             populationAM(w,:)  = GlobalPollination(populationAM(w,:),DimensionAm,gbest,lambda,type_levy);
             
          else
		     %% Local pollination --------------------------------------------------
             populationAM = LocalPollination(populationAM,indiv,DimensionAm,w);
         end
         % -----------------------------------------------------------------
